		...
		, success: function(data) {
			if(data.Code == '100'){
				...
				if(response != null && response != ''){
					var data_level = response[0].userlevel;
					if(data_level != null && data_level != ""){
						...
						userLevelArr = ['ADMIN','DELETE','MODIFY','WRITE'];
					}
				}
			}else{
...
// 			jAlert('날짜를 입력해 주십시오./n ex)YYYYMMDD','정보');
			jAlert('Please enter a date./n ex)YYYYMMDD','Info');
			return;
		}
		if($('#startDate').val() != ''){
...
// 				jAlert('날짜를 입력해 주십시오./n ex)YYYYMMDD','정보');
	...
	var param		= loginToken + "/"+ loginId +"/"+ searchType + "/"+ searchText + "/"+ sDate + "/"+ eDate + "/"+ nowPageNum + "/"+ nowSelUserNum;
	var callBack	= "?callback=?";
				...
				if(data_line_arr != null && data_line_arr != ''){
					...
					if(data_line_arr != null && data_line_arr != ""){
						for(var i=0; i<data_line_arr.length; i++) {
							...
							var tmpRdata = "";
							if(data_line_arr[i].r_date != null && data_line_arr[i].r_date != undefined){
								tmpRdata = data_line_arr[i].r_date;
							}
							innerHTMLStr += '<td align="center">'+ tmpRdata +'</td>';	//reg_date
							innerHTMLStr += '<td align="center"><select id="userLvSel_'+ i +'">';
							for(var j=0; j<userLevelArr.length;j++){
								innerHTMLStr += '<option ';
								if(userLevelArr[j] == data_line_arr[i].type.trim()){
...
function userTypeSave(){
	...
	var param		= loginToken + "/"+ loginId +"/"+ saveStr;
	var callBack	= "?callback=?";
